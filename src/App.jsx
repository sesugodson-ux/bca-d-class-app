...
export default function App(){
  ...
  const fetchStudents = useCallback(async function(){
    ...
    const formattedData = data.map(s => ({
      ...
      name: s.student_name,
      studentMob: s.mobile_number,
      fatherMob: s.father_mobile,
      collegeID: s.email,
      dob: s.date_of_birth,
      partOne: s.part_one_language
    }));
    setStudentDb(formattedData);
  }, []);
  ...
  async function addStudent(newStudent) {
    const { error } = await supabase.from('students').insert([{
      ...
      mobile_number: newStudent.studentMob,
      father_mobile: newStudent.fatherMob,
      email: newStudent.collegeID,
      date_of_birth: newStudent.dob,
      part_one_language: newStudent.partOne,
      class_name: '2 B.C.A. D',
      section: 'D',
      status: 'active'
    }]);

    if (error) { showToast('Could not add student: ' + error.message, true); return; }
  ...
  const searchResults = useMemo(function(){
    ...
    return studentDb.filter(function(s){
      ...
      const rollMatch = rollQ ? String(s.rollNo).indexOf(rollQ)!==-1 : true;
      const mobMatch = mobQ ? (String(s.studentMob).indexOf(mobQ)!==-1 || String(s.fatherMob).indexOf(mobQ)!==-1) : true;
  ...
  async function attemptStudyLogin(){
    ...
    try{
      if(studyLoginMode==='admin'){
        ...
        const student = studentDb.find(function(s){ return s.name!=='XX' && String(s.rollNo).trim()===rollNo && s.dob===dob; });
  ...
  const filteredStudents = useMemo(function(){
    ...
    return studentDb.filter(function(s){
      return s.name.toLowerCase().indexOf(q)!==-1 || String(s.rollNo).indexOf(q)!==-1 || String(s.id).indexOf(q)!==-1;
    });
